...
# 2. Инжектим пресет
echo "Добавляем пресет custom_max..."
{
...
# Принудительно меняем Factory на factory внутри самого скачанного патчера, если он там есть
sed -i 's/Factory/factory/g' /tmp/txpwr_mod.sh

mv /tmp/txpwr_mod.sh "$PATCHER_PATH"
chmod +x "$PATCHER_PATH"

# 3. Ищем раздел factory
...
# 6. Снимаем защиту записи
echo "Снимаю защиту записи с чипа памяти..."
if command -v apk >/dev/null 2>&1; then
    apk add kmod-mtd-rw 2>/dev/null
elif command -v opkg >/dev/null 2>&1; then
    opkg install kmod-mtd-rw 2>/dev/null
fi

insmod mtd-rw i_want_a_brick=1 2>/dev/null

# 7. Запись строго по MTD-устройству (mtd2), чтобы утилита mtd не гадала по имени
echo "Записываю обновленный дамп в /dev/$MTD_DEV (factory)..."
mtd write "$DUMP_FILE" "/dev/$MTD_DEV"
